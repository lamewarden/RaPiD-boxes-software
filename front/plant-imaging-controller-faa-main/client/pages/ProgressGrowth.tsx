...
export default function ProgressGrowth() {
  ...
  const phaseLabel = status?.phase
    ...
    : isActive
      ? "Starting…"
      : "—";
  const onBaseline = status?.phase === "baseline";
  const phaseIndex = status?.currentPhaseIndex ?? null;
  const phases = status?.phases ?? [];
  const prevPhase = !onBaseline && phaseIndex != null && phaseIndex > 0 ? phases[phaseIndex - 1] : null;
  const nextPhase = onBaseline
    ? phases[0] ?? null
    : phaseIndex != null && phaseIndex + 1 < phases.length
      ? phases[phaseIndex + 1]
      : null;
            ...
            >
              ...
              <p className="text-app-text-muted text-[9px]">
                {isPaused
                  ...
                  : status?.state === "done"
                    ? "Finished"
                    : `${formatElapsedShort(status?.phaseTotalSeconds ?? 0)}${dayLabel ? ` · ${dayLabel}` : ""}`}
              </p>
              {nextPhase && (
